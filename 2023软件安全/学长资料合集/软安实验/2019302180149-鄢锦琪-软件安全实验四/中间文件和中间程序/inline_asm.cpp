...
void setBytes(char* _src, int _len, long _offset_file, FILE* fp) {
    int cnt = 0;
...
void a() {
    FILE* fp = fopen("arguments.txt", "wt+");
    DWORD baseOfmsvcr120 = 0;                    //-84
    DWORD addressOfCurrent = 0;                  //-80
    ...
    setBytes((char*)&baseOfmsvcr120, 4, 0, fp);  // 4个字节
    setBytes((char*)&addressOfCurrent, 4, 4, fp);  
    setBytes(dllname, 16, 4+4, fp);
    setBytes(functionname, 16, 16 + 4 + 4, fp);
    setBytes(info, 16, 16 + 16 + 4 + 4, fp);
    setBytes(loadlibraryEx, 16, 16 + 16 + 16 + 4 + 4, fp);
    setBytes((char*)&pLoadLibraryExA, 4, 16 + 16 + 16 + 16 + 4 + 4, fp);
    setBytes((char*)&kernelBase, 4, 16 + 16 + 16 + 16 + 4 + 4 + 4, fp);
    setBytes((char*)&getProcAddr, 4, 16 + 16 + 16 + 16 + 4 + 4 + 4+ 4 , fp);
    fclose(fp);
}
